...
int numIterations = 2;//nodes' parameters are initialized by zero vectors, so in the first batch of the first iteration, the word vectors are not updated.
...
class Node {
    public:
        ...
        int count = 0;
        int location = 0;
        //std::vector<double> para = std::move(std::vector<double>(vectorSize, 0));
...
void updateNodesParas(Node* root, std::vector<std::vector<double>> paras){
    ...
    std::stack<Node*> nodes;
    int i = 0;
    if (husky::Context::get_global_tid() == 0 || husky::Context::get_global_tid() == 1){
        husky::LOG_I << husky::Context::get_global_tid() <<  " TESTING paras[0] num : " << paras[0][vectorSize];
    }
    if (husky::Context::get_global_tid() == 0){
        Node* test_node = search(root, "paris");
        if (test_node == NULL ) husky::LOG_I << NULL;
        husky::LOG_I << husky::Context::get_global_tid() <<  " TESTING paris num : " << paras[test_node->location][vectorSize];
        husky::LOG_I << "TESTING before updating paris->para[0]: " << test_node->para[0];
    }
    while (root != NULL || !nodes.empty()){
        if(root != NULL) {
            nodes.push(root);
            ...
            root = nodes.top();
            // husky::LOG_I << "TESTING root->para[0]: " << root->para[0];
            // husky::LOG_I << "TESTING paras[i][0]: " << paras[i][0];
            // husky::LOG_I << "TESTING update number: " << paras[i][vectorSize];
            if (paras[i][vectorSize] > 0){
                for (int j = 0; j < vectorSize; j++){
                    // root->para[j] += paras[i][j] / paras[i][vectorSize];
                    root->para[j] += paras[i][j] / 2;
                }
            }
            i++;
            nodes.pop();
...
void train_v2(int count, Node* root){
    ...
    for (int m = 0; m < numBatch; m++){
        ...
        list_execute(document_list, {}, {&ac}, [&](Document& doc) {
            ...
            // husky::LOG_I << "begin updating aggregator"; 
            // update_paras.update(getNodesParas(root));
            /*
            std::stack<Node*> nodes;
            int k = 0;
            while (root != NULL || !nodes.empty()){
...
*/
       ...
       */
        husky::LOG_I << "before sync";
        husky::lib::AggregatorFactory::sync();
        husky::LOG_I << "after sync";
        std::vector<std::vector<double>> paras_aggr (update_paras.get_value());
        updateNodesParas(root, paras_aggr);
        husky::LOG_I << " after updating";
        if (husky::Context::get_global_tid() == 0){
            Node* test_node = search(root, "paris");
    ...
    */
    ...
    if (husky::Context::get_global_tid() == 0){
        for (int i = 0; i < wc_list.size(); i++){
            ...
            // husky::LOG_I << "node->para[0]: " << node->para[0];
            update_paras.update_any([&](std::vector<std::vector<double>>& v){
                for (int m = 0; m < vectorSize; m++) {
                    v[i][m] = node->para[m];
                }
    ...
    /* 
    if (husky::Context::get_global_tid() == 0 || husky::Context::get_global_tid() == 1){
        husky::LOG_I << "paras_aggr[0][0] " << paras_aggr[0][0] << " tid: " << husky::Context::get_global_tid();
    }
    */
    if (husky::Context::get_global_tid() != 0){
        for (int i = 0; i < wc_list.size(); i++){
...
auto createVocabByFile(char* filename) {
    ...
    if (husky::Context::get_global_tid() == 0){ 
        ...
        char * content;
        size_t result;
        content = (char*) malloc (sizeof(char)*1e7/numPartitions);
        for (int i = 0; i < numPartitions; i++){
            if (husky::Context::get_global_tid() == 0) {
                // husky::LOG_I << "partition: " << i;
            }

            Document doc("doc" + (char)i);
            ...
            result = fread (content,1,1e7/numPartitions,file);
            ...
            // husky::LOG_I << "last word " << doc.words.back();
            // doc.batch = 0;
            doc.batch = rand() % numBatch;
            document_list.add_object(doc);
        }
